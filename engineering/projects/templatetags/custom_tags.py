...
def process_system(system, current_system, current_category):
    ...
    if not current_system and not current_category:
        return '<a href="?system=' + system['system__slug'] + '" class="filter-button btn btn-primary active" role="button">' + system['system__name'] + '</a><br>'
    if system['system__slug'] == current_system:
        return '<a href="#" class="filter-button btn btn-success btn-outline inactive" role="button">'+ system['system__name'] + '</a><br>'
    elif ControlledDocument.objects.filter(**kwargs).exists() == True:
        return '<a href="?category=' + current_category + '&system=' + system['system__slug'] + '" class="filter-button btn btn-primary active" role="button">' + system['system__name'] + '</a><br>'
    else: 
